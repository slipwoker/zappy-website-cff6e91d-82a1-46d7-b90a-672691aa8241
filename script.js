document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, ruleName, ruleValue) {
    ...
    function showError(field, message) {
      ...
      const errorId = (field.getAttribute('id') || field.getAttribute('name') || Math.random().toString(36).slice(2)) + '-error';
      ...
      if (!errorEl) {
        ...
        errorEl.classList.add('form-error-message');
    ...
    function validateField(field) {
      ...
      for (const rule of rules) {
        ...
        const isValid = validatorFn(field.value, ruleValue);
        if (!isValid) {
    ...
    function getValidatableFields() {
      return Array.from(contactForm.querySelectorAll('input, textarea, select')).filter(
        (field) => !['hidden', 'submit', 'reset', 'button', 'image'].includes(field.type)
      );
    }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      fetch(action, {
        ...
        .catch(function () {
          const errorBanner = contactForm.querySelector('.form-submit-error');
          if (errorBanner) {
    ...
    contactForm.addEventListener('blur', function (e) {
      ...
      if (['input', 'textarea', 'select'].includes(field.tagName.toLowerCase())) {
        validateField(field);
      }
    }, true);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "אחד הטעימים משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'cff6e91d-82a1-46d7-b90a-672691aa8241',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'cff6e91d-82a1-46d7-b90a-672691aa8241';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'cff6e91d-82a1-46d7-b90a-672691aa8241';
